...
void appWebServer(void) {
    ...
    server.on("/api/v1/sense_config", HTTP_POST, postSenseConfig);
...
static void postSenseConfig() {
    ...
    String content = server.arg("plain");
    log_d("POST /api/v1/sense_config content: '%s'", content.c_str());
    reqObject = cJSON_Parse(content.c_str());
    if (reqObject == NULL) {
        log_w("JSON parse error");
        log_w("payload: '%s'", server.arg("plain").c_str());
    ...
    senseObject = cJSON_GetObjectItem(reqObject, "sense");
    if (senseObject) {
        cJSON *item;
        item = cJSON_GetObjectItem(senseObject, "endpoint");
        if (item) db.sense.endpoint = item->valuestring;
        item = cJSON_GetObjectItem(senseObject, "api_key");
        if (item) db.sense.apiKey = item->valuestring;
        item = cJSON_GetObjectItem(senseObject, "device_id");
        if (item) db.sense.deviceId = item->valuestring;
        item = cJSON_GetObjectItem(senseObject, "latitude");
        if (item) db.sense.latitude = (float)item->valuedouble;
        item = cJSON_GetObjectItem(senseObject, "longitude");
        if (item) db.sense.longitude = (float)item->valuedouble;
        item = cJSON_GetObjectItem(senseObject, "location_label");
        if (item) db.sense.locationLabel = item->valuestring;
        item = cJSON_GetObjectItem(senseObject, "country_code");
        if (item) db.sense.countryCode = item->valuestring;
        db.factoryState = false;
    }

    server.send(200, "application/json", server.arg("plain"));
    log_d("POST /api/v1/sense_config response: '%s'", server.arg("plain").c_str());
...
static void postConfig() {
    ...
    senseObject = cJSON_GetObjectItem(configObject, "sense");
    if (senseObject) {
        cJSON *item;
        item = cJSON_GetObjectItem(senseObject, "endpoint");
        if (item) db.sense.endpoint = item->valuestring;
