...
typedef enum UploadState_t {
    E_UPLOAD_STATE_INIT = 0,
    E_UPLOAD_STATE_SUCCESS,
    E_UPLOAD_STATE_FAILURE,
} UploadState_t;
...
UploadState_t uploadState = E_UPLOAD_STATE_INIT;
...
void setup() {
    ...
    log_i("Project name: AirQ Sense Platform");
    ...
    // Configure Sense Platform from persisted config
...
void mainApp(ButtonEvent_t *buttonEvent) {
    ...
    static bool runningUpload = false;
    static int uploadRetryCount = EZDATA_UPLOAD_RETRY_COUNT;
    ...
    if (refresh) {
        ...
        if (
            ...
            && WiFi.isConnected()
            && db.sense.endpoint.length() > 0
        ) {
            uploadRetryCount = EZDATA_UPLOAD_RETRY_COUNT;
            runningUpload = true;
        }
    }
    ...
    if (WiFi.isConnected() && runningUpload && uploadRetryCount-- > 0) {
        // Re-apply config in case it was changed via web UI
        sensePlatform.setEndpoint(db.sense.endpoint);
        sensePlatform.setApiKey(db.sense.apiKey);
        sensePlatform.setDeviceId(db.sense.deviceId);
        if (db.sense.latitude != 0 || db.sense.longitude != 0) {
        ...
        if (sensePlatform.upload(
            ...
            SUCCESS_TONE();
            runningUpload = false;
            uploadState = E_UPLOAD_STATE_SUCCESS;
        } else {
            failCounter += 1;
